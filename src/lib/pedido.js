...
/** Pipeline comercial de pedidos (Order Bridge).
 *  Web: recibido → confirmado → pendiente_carga → enviado_bodega → cargado
 *  Campo: borrador → enviado → confirmado → …
 */
export const PIPELINE_PEDIDO = [
  { id: 'recibido', label: 'Nuevo web', color: '#0d9488' },
  { id: 'borrador', label: 'Borrador', color: '#a8a29e' },
  { id: 'enviado', label: 'Enviado', color: '#2563eb' },
  { id: 'confirmado', label: 'Confirmado', color: '#15803d' },
  { id: 'pendiente_carga', label: 'Pendiente carga', color: '#d97706' },
  { id: 'enviado_bodega', label: 'En bodega', color: '#7c3aed' },
  { id: 'cargado', label: 'Cargado', color: '#0f766e' },
  { id: 'cancelado', label: 'Cancelado', color: '#b91c1c' },
]

export function siguienteEstadoPipeline(estado) {
  const e = String(estado || '').toLowerCase()
  const map = {
    recibido: 'confirmado',
    borrador: 'enviado',
    enviado: 'confirmado',
    confirmado: 'pendiente_carga',
    pendiente_carga: 'enviado_bodega',
    enviado_bodega: 'cargado',
  }
  return map[e] || null
}

/** Pedidos web pendientes de atención (inbox del ejecutivo). */
export async function listarInboxPedidos(ejecutivoId, { dias = 14, limit = 40 } = {}) {
  if (!ejecutivoId) return { data: [], error: null }
  const desde = new Date()
  desde.setDate(desde.getDate() - dias)
  desde.setHours(0, 0, 0, 0)
  ...
  const rows = data || []
  // Inbox = web nuevos + confirmados aún no cargados
  const inboxEstados = new Set(['recibido', 'enviado', 'confirmado', 'pendiente_carga', 'enviado_bodega'])
  const inbox = rows.filter(p => {
    const fuente = String(p.fuente || '')
    const est = String(p.estado || '').toLowerCase()
    if (fuente === 'catalogo_publico') {
      return !['cargado', 'cancelado'].includes(est)
    }
    // también mostrar terreno en estados activos intermedios si el usuario quiere "todos"
    return false
  })
  return { data: inbox, error: null, all: rows }
}

export async function listarPedidosWebNuevos(ejecutivoId) {
  const { data, error } = await listarInboxPedidos(ejecutivoId)
  if (error) return { data: [], error }
  const nuevos = (data || []).filter(p => String(p.estado || '').toLowerCase() === 'recibido')
  return { data: nuevos, error: null, inbox: data }
}

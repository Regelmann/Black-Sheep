        ...
        rows.append(
            {
                ...
                "precio_unidad": meta.get("precio_unidad"),   # ← NUEVO: precio lista de precios
                "precio_kilo": _f(meta.get("kg_unidad")) and meta.get("precio_unidad") and
                               round(meta["precio_unidad"] / meta["kg_unidad"], 0) if (
                                   meta.get("precio_unidad") and meta.get("kg_unidad") and
                                   float(meta.get("kg_unidad", 0)) > 0
                               ) else None,
                "fecha_snapshot": date.today().isoformat(),
            }
        )
...
def merge_ventas_incremental(excel_df: pd.DataFrame, sb, mes_inicio: date) -> pd.DataFrame:
    ...
    # ANTI-DOBLE CONTEO: Excel con historia larga = fuente del periodo.
    # Hist Supabase solo suma días ESTRICTAMENTE posteriores al max del Excel.
    # Si no, el MTD se infla (ej. Zona Sur $22M → $32M, pollo 2×).
    if hist is not None and not hist.empty and "fecha_d" in getattr(hist, "columns", []) and span_days >= 60:
        xmax = excel_df["fecha_d"].max()
        before_h = len(hist)
        hist = hist[hist["fecha_d"] > xmax].copy()
        print(
            f"  anti-doble: Excel cubre {span_days}d hasta {xmax} → "
            f"hist solo > max Excel: {before_h} → {len(hist)} filas"
        )

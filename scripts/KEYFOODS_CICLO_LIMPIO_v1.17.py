...
# ---------------------------------------------------------------------------
# PATHS — ajustar en Colab / local / GitHub Actions
# ---------------------------------------------------------------------------
# KF_DATA_DIR: variable de entorno para GitHub Actions (apunta a ./data/)
# En Colab: las rutas /content/drive/... siguen funcionando igual
_env_data_dir = os.environ.get("KF_DATA_DIR", "").strip()
SEARCH_DIRS = (
    [Path(_env_data_dir)]          # GitHub Actions → ./data/
...
DEFAULT_ZONAS_COMUNAS = {
    ...
    "NOR-PONIENTE": [
        "RECOLETA", "INDEPENDENCIA", "HUECHURABA",
...
def load_config_mensual(path: Optional[Path]) -> Tuple[List[dict], List[dict], Dict[str, str]]:
    ...
    # --- FOCO_SKU (sku → nombre de foco + factor KG) ---
    foco_skus: Dict[str, str] = {}
    foco_sku_kg: Dict[str, float] = {}   # sku → kg por unidad (fuente más confiable)
    ...
    if sdf is not None and not sdf.empty:
        ...
        c_kg   = sc(
            ...
            "factor_kg", "kg_unidad", "kg_por_unidad",
            "kilos_unidad", "kg_equivalente",
        )
        if c_sku:
        ...
        print(f"  config FOCO_SKU: {len(foco_skus)} skus, {len(foco_sku_kg)} con factor KG")
...
def merge_ventas_incremental(excel_df: pd.DataFrame, sb, mes_inicio: date) -> pd.DataFrame:
    ...
    # Seguridad: MTD no debería caer >25% vs lo ya publicado en gerencia (si existe)
    try:
        mes_fin = (mes_trabajo.replace(day=28) + timedelta(days=4)).replace(day=1)
        mtd_new = float(
            both[(both["fecha_d"] >= mes_trabajo) & (both["fecha_d"] < mes_fin)]["venta_neta_clp"]
            .fillna(0).sum()
        )
        prev = sb.table("gerencia").select("venta_mtd").execute().data or []
        mtd_prev = sum(float(x.get("venta_mtd") or 0) for x in prev)
        print(f"  control MTD: nuevo=${mtd_new:,.0f} | gerencia previa suma=${mtd_prev:,.0f}")
        if mtd_prev > 0 and mtd_new < mtd_prev * 0.75 and not FORCE_VENTAS:
            raise SystemExit(
                f"ABORT: MTD nuevo (${mtd_new:,.0f}) es <75% del publicado (${mtd_prev:,.0f}).\n"
                f"  El Excel no trae el mes completo o el histórico está incompleto.\n"
                f"  Revisá el archivo o usá KF_FORCE_VENTAS=1 si es intencional."
            )
    except SystemExit:
        raise
    except Exception as e:
        print(f"  control MTD (skip): {str(e)[:120]}")
...
def main():
    ...
    metas_cfg, focos_cfg_mes, foco_sku_map, foco_sku_kg = load_config_mensual(p_config)
    ...
    # Mapa sku → kg por unidad
    # Fuente 1 (más confiable): hoja FOCO_SKU del archivo de configuración mensual
    # Fuente 2 (fallback): lista de precios
    kg_por_sku: Dict[str, float] = {}

    # Fuente 1: config mensual FOCO_SKU (factor_kg_equivalente_por_unidad)
    for sk, factor in (foco_sku_kg or {}).items():
        if factor and factor > 0:
            kg_por_sku[normalize_sku(sk)] = float(factor)
    if foco_sku_kg:
        print(f"  kg_por_sku config (FOCO_SKU): {len(foco_sku_kg)} factores explícitos")

    # Fuente 2: lista de precios (solo para SKUs que no tienen factor en config)
    if precios is not None and not precios.empty:
        for _, pr in precios.iterrows():
            sk = normalize_sku(pr.get("sku_canon") or pr.get("sku") or "")
            if not sk or sk in kg_por_sku:
                continue
            kgu = _f(pr.get("kg_unidad"))
            if kgu and kgu > 0:
                kg_por_sku[sk] = float(kgu)
            else:
                kgc = _f(pr.get("kg_caja"))
                uc = _f(pr.get("unidades_caja"))
                if kgc and uc and uc > 0:
                    kg_por_sku[sk] = float(kgc) / float(uc)
    print(f"  kg_por_sku total: {len(kg_por_sku)} skus con factor KG")

    def _kg_line(row) -> float:
        """Convierte una línea de venta a KG (o LT si no hay factor)."""
        cant = float(row.get("cantidad") or 0)
        if cant == 0:
            return 0.0
        sk = str(row.get("sku_canon") or "")
        kgu = kg_por_sku.get(sk)
        if kgu and kgu > 0:
            return cant * kgu
        # sin factor: si el nombre sugiere ya kg, usar cantidad; si no, cantidad cruda
        return cant

    for f in focos_cfg_mes:
        z = f.get("zona") or "OTROS"
        foco_name = f.get("foco") or ""
        unidad_meta = str(f.get("unidad_meta") or "KG").upper()
        # SKUs de este foco (mapa config)
        skus_foco = [sk for sk, fn in (foco_sku_map or {}).items() if _norm_col(fn) == _norm_col(foco_name)]
        # Fallback: match por nombre de producto si el mapa está vacío o incompleto
        if not skus_foco and not vm_f.empty and "producto_nombre" in vm_f.columns:
            token = _norm_col(foco_name)
            # tokens útiles: POLLO, HANKS, SALSA, etc.
            toks = [t for t in token.replace("_", " ").split() if len(t) >= 3]
            if toks:
                mask_n = vm_f["producto_nombre"].map(
                    lambda x: any(t in _norm_col(str(x or "")) for t in toks)
                )
                skus_foco = sorted({str(s) for s in vm_f.loc[mask_n, "sku_canon"].dropna().unique()})
                print(f"  foco {z}/{foco_name}: fallback nombre → {len(skus_foco)} skus")
        vendido = 0.0
        if not vm_f.empty and skus_foco:
            sub = vm_f[vm_f["sku_canon"].isin(skus_foco)].copy()
            # filtrar por zona del cliente (maestra) o zona_vendedor
            if "cliente_key" in sub.columns:
                mask_z = sub["cliente_key"].map(
                    lambda ck: normalize_zona(zona_por_ck.get(str(ck), "")) == z
                )
                sub2 = sub[mask_z]
                # Fallback: zona del vendedor en la factura
                if "zona_vendedor" in sub.columns:
                    sub_v = sub[sub["zona_vendedor"].map(normalize_zona) == z]
                    if sub2.empty or len(sub_v) > len(sub2):
                        sub2 = sub_v
                sub = sub2
            if not sub.empty:
                if "cantidad" in sub.columns:
                    # Meta en KG/LT → convertir; si unidad es UD sumar cantidad cruda
                    if "KG" in unidad_meta or "LT" in unidad_meta or "L" == unidad_meta:
                        vendido = float(sub.apply(_kg_line, axis=1).sum())
                    else:
                        vendido = float(sub["cantidad"].fillna(0).sum())
                else:
                    vendido = float(sub["venta_neta_clp"].fillna(0).sum())
        ...
        print(
            f"  foco {z}/{foco_name}: skus={len(skus_foco)} "
            f"vendido={vendido:.1f} {unidad_meta} meta={meta_u} → {ritmo}"
        )

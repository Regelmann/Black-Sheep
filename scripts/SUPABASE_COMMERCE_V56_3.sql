-- =============================================================================
-- BLACK SHEEP / KEYFOODS COMMERCE V56.3 (+ patch pedidos vendedor)
-- Ejecutar UNA vez en Supabase SQL Editor (service role no hace falta).
-- =============================================================================

create extension if not exists pgcrypto;

-- 1) Stock: precio maestro publicado por el CICLO desde LISTA DE PRECIOS
...
-- 2) Oferta permanente por cliente
...
create index if not exists oferta_items_oferta_idx on public.oferta_cliente_items (oferta_id);

-- Pedidos: columnas útiles para vendedor
...
-- Políticas básicas (ejecutivo autenticado gestiona sus ofertas)
...
-- 3) Catálogo público (anon vía RPC)
...
-- 4) Pedido público → visible para el ejecutivo dueño de la oferta
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lineas) loop
    ...
    -- fallback stock
    if v_price <= 0 then
      select coalesce(nullif(precio_unidad,0), nullif(precio_caja,0), 0)
        into v_price from public.stock where sku_canon = v_sku limit 1;
...
-- Ejecutivo ve pedidos de su catálogo (fuente catalogo_publico)
-- (RLS de pedidos debe permitir select por ejecutivo_id = auth.uid())

-- RPC catálogo con columnas REALES de oferta_cliente_items
-- (sin recomendado / categoria — usa destacado)
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(x ORDER BY COALESCE((x->>'prioridad')::int, 0), x->>'producto_nombre'), '[]'::jsonb)
    INTO v_items
  FROM (
...
SELECT public.get_public_catalogo(
  (SELECT token FROM public.ofertas_cliente WHERE COALESCE(activo, true) LIMIT 1)
) AS prueba_rpc;

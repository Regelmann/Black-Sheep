...
33_ALINEAR_MAESTRA_ZONAS.sql    ← alinea zona con comuna en la maestra
...
40_STOCK_COLUMNAS_CICLO.sql     ← stock acepta las 26 columnas del ciclo
41_VENTAS_LINEAS.sql            ← 🔴 la tabla que hace funcionar el incremental
42_ZONAS_CONFIGURABLES.sql      ← 🔴 zonas como DATOS, no código (base de replicación)
43_CHECKINS_COLUMNAS.sql        ← 🔴 arregla el "Error al sincronizar" pegado en toda la app
49_PREFLIGHT_MULTITENANT.sql    ← 🔴 PRIMERO: claves compuestas (sin esto el 2do tenant no entra)
50_FUNDACION_MULTITENANT.sql    ← CIMIENTO: tenant_id + índices
security/51_MULTITENANT_RBAC.sql ← RBAC fail-closed (sin fallback a keyfoods)
security/52_RLS_POLICIES.sql     ← políticas de aislamiento
security/53_STORAGE_RLS.sql      ← aislamiento de archivos subidos
security/99_SECURITY_DIAGNOSTICS.sql ← verificar que no quedó nada abierto
43_CHECKINS_ESQUEMA.sql         ← 🔴 arregla "Error al sincronizar" en toda la app
...
2. Abrir `app.black-sheep.cl` → el stamp abajo debe decir **`v-BS-PLATFORM-V15.3`**
...
[ ] 3 · git push
[ ] 4 · Stamp en pantalla dice v-BS-PLATFORM-V15.3

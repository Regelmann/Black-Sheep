...
export function revisar({ raiz, core }) {
  ...
  for (const f of todos) {
    ...
    if (/\.select\(\s*['"`]\*/.test(txt)) fallas.push(`${rel(f)}: select('*')`)
    ...
    if (f.endsWith('.css')) {
      ...
      const esperados = permitido
        ? (bruto.match(/reduce\)[\s\S]{0,600}?\}\s*\}/)?.[0].match(/!important/g) || []).length
        : 0
      if (usos > esperados) fallas.push(`${rel(f)}: ${usos - esperados} !important fuera de la excepción`)
    }
  ...
  //      no aplica. Así `--fuente` faltó y todo salió en serif.
  {
    ...
    for (const f of todos.filter((x) => x.endsWith('.css'))) {
      for (const m of readFileSync(f, 'utf8').matchAll(/^\s*(--[a-z0-9-]+)\s*:/gm)) definidas.add(m[1])
    }
    for (const f of todos.filter((x) => x.endsWith('.css'))) {
      for (const m of readFileSync(f, 'utf8').matchAll(/var\((--[a-z0-9-]+)/g)) {
        if (!definidas.has(m[1])) fallas.push(`${rel(f)}: usa ${m[1]}, que no está definida`)
      }
    }
  }
  ...
  //      pantalla de Empresas quedó como texto plano sin avisar.
  {
    ...
    for (const f of todos.filter((x) => x.endsWith('.css'))) {
      for (const m of readFileSync(f, 'utf8').matchAll(/\.([a-z][a-z0-9-]*)/g)) clases.add(m[1])
    }
    for (const f of archivos(raiz).filter((x) => x.endsWith('.jsx'))) {
      ...
      for (const c of usadas) {
        if (!clases.has(c) && !MODIFICADORES.has(c))
          fallas.push(`${rel(f)}: usa la clase .${c}, que ninguna hoja define`)
      }
    }
  }
  ...
  for (const f of archivos(raiz).filter((x) => x.endsWith('.jsx'))) {
    const n = (readFileSync(f, 'utf8').match(/<Kpi\s/g) || []).length
    if (n > 4) fallas.push(`${rel(f)}: ${n} tarjetas de KPI, el máximo es 4`)
  }

  return { fallas, revisados: todos.length }
}

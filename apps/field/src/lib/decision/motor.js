/**
 * MOTOR DE DECISIÓN · un solo cerebro
 *
 * Portado de la V15.4 con dos correcciones para la 2.0:
 *
 * 1. ES PURO. No importa Supabase ni React: recibe datos y devuelve
 *    decisiones. Por eso se puede probar sin base y funciona sin señal,
 *    que es donde vive el vendedor la mitad del día.
 *
 * 2. NO CALCULA SUS ENTRADAS. `ciclo_dias`, `venta_mensual` y
 *    `estado_fuga` llegan del servidor (api.decision_cartera, migración
 *    034). En la 15.4 se deducían acá, y por eso el mismo cliente salía
 *    "en riesgo" en el teléfono y "al día" en el dashboard: cada
 *    pantalla elegía su umbral. Ahora la definición vive una sola vez.
 *
 * QUÉ DEVUELVE
 * Una decisión no es una alerta: es una acción con su porqué.
 *   type          replenish · protect · order · focus
 *   attention     now · today · week
 *   score 0-100   urgencia .30 valor .25 probabilidad .20
 *                 accionable .15 confianza .10
 *   why[]         las razones, en castellano, para que el vendedor
 *                 pueda repetirlas frente al cliente
 *   evidence[]    los números que las respaldan
 *   actionLabel   qué botón mostrar
 *   expectedValue cuánta plata hay en juego
 *
 * El peso de `accionable` existe por algo: una decisión sin acción
 * posible en terreno no sirve, por urgente que sea.
 */
import { calcGoal } from '../goal.js'
...
export function decideClient(c) {
  ...
  const nombre = c.nombre || c.nombre_cliente || c.razon_social || String(key)

  // El estado lo decide el SERVIDOR (core.v_cliente_ritmo), que lo
  // calcula contra el ciclo propio del cliente. El motor lo respeta en
  // vez de deducirlo de los días: es la misma razón por la que las
  // métricas viven en una vista y no acá.
  const enFuga = /EN_FUGA|PERDIDO|DORMIDO|FUGADO|RIESGO|ENFRI/.test(estado)

  // --- Rescate · va ANTES que reposición ---
  // Entre 30 y 45 días los dos rangos se solapan. Si gana reposición,
  // a un cliente que ya se está yendo se le ofrece "armar pedido" como
  // si fuera una visita de rutina, y se pierde el momento de
  // rescatarlo.
  if (enFuga && dias != null && dias >= 21 && dias <= 90) {
    const urgencia = Math.min(100, 45 + (90 - dias) * 0.4 + (venta >= 300000 ? 15 : 0))
    ...
    if (att) return {
      ...
      why: [
        `${dias}d sin compra` + (ciclo > 0 ? ` · compra cada ~${ciclo}d` : ''),
        venta > 0 ? `Valía ${money(venta)} al mes` : 'Marcado en riesgo',
        'Todavía recuperable',
  ...
  // (El rescate se evalúa arriba, antes de reposición.)

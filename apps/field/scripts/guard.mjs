...
// El propio guard contiene los patrones que busca.
// Por eso no debe escanearse a sí mismo.
...
for (
  const f of [...archivos(raiz), ...archivos(core)]
    .filter((f) => !f.startsWith(guardDir))
) {
  ...
  // R7 · columnas explícitas.
  // Un select('*') trae de más y se rompe en silencio cuando la vista cambia.
  if (/\.select\(\s*['"`]\*/.test(txt)) {
    fallas.push(`${rel}: select('*')`)
  }

  // R8 · ningún secreto en el repositorio.
  if (
    /service_role|SERVICE_KEY|eyJhbGciOiJIUzI......................../.test(txt)
  ) {
  ...
  if (f.endsWith('.css')) {
    ...
    if (usos > esperados) {
      fallas.push(
        `${rel}: ${usos - esperados} !important fuera de la excepción`
      )
    }
  }
  ...
  if (/data:image\/svg\+xml[^`'"]*var\(--/.test(txt)) {
    fallas.push(
      `${rel}: var(--…) dentro de un SVG embebido (sale negro)`
    )
  }
  ...
  if (/rpc\([^)]*p_tenant/.test(txt) && !/admin_/.test(txt)) {
    fallas.push(
      `${rel}: manda p_tenant a una función que no es de superadmin`
    )
  }
}
...

{
  const cssTokens = readFileSync(
    fileURLToPath(
      new URL('../../../packages/marca/tokens.css', import.meta.url)
    ),
    'utf8'
  )
    ...
    for (
      ...
      if (!definidas.has(m[1])) {
        fallas.push(
          `${f
            .replace(raiz, 'src')
            .replace(core, 'packages')}: usa ${m[1]}, que no está definida en tokens.css`
        )
      }
    }
  }
...
console.log(
  `Guard: ${
    archivos(raiz).length + archivos(core).length
  } archivos revisados, sin hallazgos.`
)